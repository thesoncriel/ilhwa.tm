...
namespace Artn.Ilhwa.Controller
{
	...
	public class DataViewBinder
	{
		...
		public const string TABLE_SEL_SPEC = 	"table_select find_task_order_spec";
		public const string TABLE_SEL_ODVALUE = "table_select find_task_order_odvalue";
		public const string TABLE_SEL_THK =		"table_select find_task_order_thickness";
		public const string TABLE_SEL_QUAL =	"table_select find_task_order_quality";
		...
		public void BindingViewLabelValue(System.Windows.Forms.Control container, IMultiColumnRecordable data){
			BindingViewLabelValue(container, data, null, true);
		}
		...
		// Printer 기능 변경 - 12.09.23 jhson
		public void BindListData(ComboBox cmbBox, string tableKey, bool addAllMark){
			DataRowCollection rows = Data.Tables[tableKey].Rows;
			bool bHasSecondCol = Data.Tables[tableKey].Columns.Count > 1;
			...
			if (bHasSecondCol == true){
				foreach(DataRow row in rows){
					dic[row[0].ToString()] = row[1].ToString();
				}
			}
			else{
				foreach(DataRow row in rows){
					dic[row[0].ToString()] = row[0].ToString();
				}
			}
