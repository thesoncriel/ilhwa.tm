...
namespace Artn.Ilhwa.Controller
{
	...
	public class TaskOrderController: AbsViewController
	{
		...
		public DataTable SearchByViewData(TaskOrder viewData){
			return LoadTable("table_select find_task_order_pop", viewData);
		}
		
		public void InsertPublishedInfo(List<IMultiColumnRecordable> data){
			foreach(IMultiColumnRecordable vd in data){
				ViewBinder.DBManager.ExecuteNonQuery("table_update change_task_order_published", vd);
			}
		}
	}
}
