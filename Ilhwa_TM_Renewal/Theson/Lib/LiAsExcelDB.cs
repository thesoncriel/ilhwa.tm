...
namespace LiAsExcelDatabase
{
    public class LiAsExcelDB
    {
    	...
    	private  DataSet DS;
    	private  bool ExistDel;
    	private  bool OldExcel;
        ...
        /// </param>
        private void SaveExcel(string FileName, DataSet DS, bool ExistDel, bool OldExcel)
        {
        	result = true;
            this.FileName = FileName;
            this.DS = DS;
            this.ExistDel = ExistDel;
            this.OldExcel = OldExcel;
            TdSave = new Thread(new ThreadStart(SaveExcel_Lootine));
            TdSave.Start();
        }
        public void SaveExcel_Lootine(){
            InitProgressValue();
            ...
            const String DEFAULT_DATA_TYPE = "CHAR(255)";
            ...
            try
            {
                ...
                foreach (DataTable DT in DS.Tables)
                {
                    ...
                    List<bool> dataTypeList = new List<bool>();
                    ...
                    foreach (DataColumn Column in DT.Columns)
                    {
                        ...
                        FldsInfo.Append("[" + Column.ColumnName.Replace("'", "''") + "] " + DEFAULT_DATA_TYPE);
                        Flds.Append( "[" + Column.ColumnName.Replace("'", "''") + "]" );
                    }
                    // Table Create
                    Cmd = new OleDbCommand("CREATE TABLE " + TableName + "(" + FldsInfo.ToString() + ")", OleDBConn);
                    //SaveText("CREATE TABLE " + TableName + "(" + FldsInfo.ToString() + ")");
                    Cmd.ExecuteNonQuery();
                    ...
                    foreach (DataRow DR in DT.Rows)
                    {
                        ...
                        Cmd = new OleDbCommand(
                            ...
                            OleDBConn);
//                        SaveText("INSERT INTO [" + TableName + "$]"+
//                            "(" + Flds.ToString() + ") "+
//                            "VALUES (" + Values.ToString() + ")");
                        Cmd.ExecuteNonQuery();
                        currentValue++;
                        //progress.SetValue((int)(((double)nowCount / (double)totalCount) * 100.0));
                        //SaveText( nowCount + " - " + totalCount + "" + (int)(((double)nowCount / (double)totalCount) * 100.0));
                    }
                    //progress.Close();
        ...
        public  int GetProgressValue(){
        	int result = (int)(  ((double)currentValue / (double)maxValue) * 100.0 );
        	if(result < 0) result = 0;
        	//SaveText(result + "");
        	return result;
        }
//        protected  void SaveText(String data){
//        	StreamWriter sw = File += Text(@"C:\haha.txt");
//            sw.WriteLine(data);
//            sw.Close();
//        }
        ...
        ///    저장할 대상 DataSet 입니다.
        /// </param>
        public void SaveExcelDB(string ExcelFile, DataSet DS)
        {
            SaveExcel(ExcelFile, DS, true, true);
        }
        public void SaveExcelDB(string ExcelFile, DataSet DS, bool oldExcel)
        {
            SaveExcel(ExcelFile, DS, true, oldExcel);
        }
