...
namespace Artn.Ilhwa.Controller.Barcode
{
	...
	public class BarcodePrinter: BarcodeSerialPort
	{
		...
		public void Print_1stProc(IMultiColumnRecordable data){
			MBarCode_Print(
				data["Customer"],
				data["ProdName1"], 
				data["ODName1"] + " * " + data["Thickness1"] + " x " + data["PlanLength"],//주문길이-->지시길이로 변경 120911 jhson
				data["ProductCode"],
				data["DeliveryDate"],
		...
		public void Print_PlanProduct(IMultiColumnRecordable data){
			MBarCode_Print(
				data["Customer"],
				data["PlanProdName"], 
				data["PlanODName"] + " * " + data["PlanThickness"] + " x " + data["PlanLength"],//주문길이-->지시길이로 변경 120911 jhson
				data["ProductCode"],
				data["DeliveryDate"],
				data["TotalQuantity"], // Quantity와 BunchCount를 곱한 값.
				data["Quantity"],
		...
		public void Print_RemainProduct(IMultiColumnRecordable data){
			MBarCode_Print(
				data["Customer"],
				data["Prod_Name"], // 2차투입잔량의 강종은 본래 제품의 것을 가져다 쓴다.
				data["ODName"] + " * " + data["Thickness"] + " x " + data["Length"],//주문길이-->지시길이로 변경 120911 jhson
				data["ProductCode"],
				data["DeliveryDate"],
				data["TotalQuantity"], // Quantity와 BunchCount를 곱한 값.
				data["Quantity"],
		...
		// B012 - 
		protected virtual void MBarCode_Print(
            string sCustName, 
            string sProdName, 
            string sSpec, 
	...
	//                sSerialData += "^PQ1^XZ" + Environment.NewLine;
	            ...
	            //sbSerialData.Append("^BY2,1,4,10" + Environment.NewLine);
	            sbSerialData.Append("^BY2,1,4,30" + Environment.NewLine);
